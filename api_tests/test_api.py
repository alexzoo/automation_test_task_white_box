...
"""
- Не верно реализованы возможности функции Range(есть выпавшие граничные значения).
- Не верная обработка отрицательных значений, если не указан параметр шкалы.
- Некорректная обработка значений выше(ниже) граничных. код ответа = 200 and status = unknown(ожидаемый код ответа 403)
- Неправильные теги для шкал Kelvin и Celsius.
- Неверная обработка значений с длинной строки более 11 символов. 
- Неверная обработка значений со знаком '+'.
- Неверная обработка десятичных значений.
- Неверно обрабатывает значение None.
- Неверная обработка значений, в которых шкала указана с маленькой буквы.
- Неверная обработка метода POST HTTP протокола.
...
class TestServerFunctionality(TestBaseClass):
    ...
    @pytest.mark.parametrize("method",
                             [
                                 requests.put,
                                 requests.post,                     # Incorrect processing of methods supported by HTTP Protocol
                                 requests.delete
                             ])
    def test_requests(self, method):
        response = method(self.url, params={'temperature': '10'})
...
class TestDefaultFunctionality(TestBaseClass):
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('1000000', 403, invalid_params),  # Incorrect processing values above the boundary
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('-1000000', 403, invalid_params)  # Incorrect handling negative values
                             ])
...
class TestKelvinFunctionality(TestBaseClass):
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('1000C', 403, invalid_params),    # Incorrect processing values above the boundary
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('-1C', 403, invalid_params)       # Incorrect processing values above the boundary
                             ])
...
class TestCelsiusFunctionality(TestBaseClass):
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('1000000K', 403, invalid_params), # Incorrect processing values above the boundary
...
class TestFahrenheitFunctionality(TestBaseClass):
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('100000F', 403, invalid_params),  # Incorrect processing values above the boundary
    ...
    @pytest.mark.parametrize("default_temp, expected_response_code, condition",
                             [
                                 ...
                                 ('-10000F', 403, invalid_params)   # The scale does not work completely
                             ])
